 ...
 */
	...
	public Solution getCentroid(){
		...
		if(solutionsList_.size() != 0){
			...
			System.out.println("in core solutionset, solutionsList size == 0");
			System.exit(0);
		}
		...
		for (int m = 0; m < objectNumb; m++) {
			...
			double value2 = 0;
			for (int k = 0; k < solutionsList_.size(); k++) {
				value1 += solutionsList_.get(k).getIthTranslatedObjective(m);
				value2 += solutionsList_.get(k).getNormalizedObjective(m);
			}
			value1 = value1 / solutionsList_.size();
			value2 = value2 / solutionsList_.size();
			sol.setIthTranslatedObjective(m, value1);
			sol.setNormalizedObjective(m, value2);
		}
