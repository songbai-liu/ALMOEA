...
public abstract class Experiment {
	...
	public Settings[] algorithmSettings_; // Paremeter experiments.settings of
											// each algorithm
	...
	public void runExperiment(int numberOfThreads) throws JMException,
		...
		System.out.println("Experiment: creating " + numberOfThreads
				+ " threads");
		System.out.println("Experiment: Number of algorithms: "
				+ algorithmNameList_.length);
		System.out.println("Experiment: Number of problems: "
				+ problemList_.length);
		System.out.println("Experiment: runs: " + independentRuns_);
		System.out.println("Experiment: Experiment directory: "
				+ experimentBaseDirectory_);
	...
	public void runCompleteExperiment(int numberOfThreads) throws JMException,
			IOException {
		this.initExperiment();
	...
	/**
	 * Especifies the experiments.settings of each algorith. This method is
	 * checked in each experiment run
	 * 
	 * @param problem
	 *            Problem to solve
	 * @param problemId
	...
	private void checkParetoFronts() {
		if (paretoFrontDirectory_.equals("") || paretoFrontDirectory_ == null) {
			...
			for (int i = 0; i < problemList_.length; i++) {
				if (paretoFrontFile_[i] == null
						|| paretoFrontFile_[i].equals("")) {
					paretoFrontFile_[i] = "";
					generateReferenceFronts(i);
				} else {
					String filePath = paretoFrontDirectory_ + "/"
							+ paretoFrontFile_[i];
					...
					if (!f.exists()) {
						...
						frontPath_[i] = paretoFrontDirectory_ + "/"
								+ paretoFrontFile_[i];
					} // if
				} // if
			} // for
					...
					for (String anIndicatorList_ : indicatorList_) {
						...
						for (int numRun = 0; numRun < independentRuns_; numRun++) {
							...
							if (!qualityIndicatorFile.equals(problemDirectory)) {
								FileWriter os;
								try {
									os = new FileWriter(qualityIndicatorFile,
											true);
									os.write("" + value + "\n");
									os.close();
		...
		if (f.exists()) {
			...
			if (f.isDirectory()) {
				System.out.println("File " + file
						+ " is a directory. Deleting directory.");
				if (f.delete()) {
	...
	public void generateReferenceFronts(int problemIndex) {
		...
		if (!rfDirectory.exists()) { // Si no existe el directorio
			boolean result = new File(referenceFrontDirectory).mkdirs(); // Lo
																			// creamos
			System.out.println("Creating " + referenceFrontDirectory);
		}

		frontPath_[problemIndex] = referenceFrontDirectory + "/"
				+ problemList_[problemIndex] + ".rf";
		...
		for (String anAlgorithmNameList_ : algorithmNameList_) {
			...
			String problemDirectory = experimentBaseDirectory_ + "/data/"
					+ anAlgorithmNameList_ + "/" + problemList_[problemIndex];
	...
	public void generateReferenceFronts() {
		...
		for (int problemIndex = 0; problemIndex < problemList_.length; problemIndex++) { // Por
																							// cada
																							// problema,
																							// generamos
																							// el
	...
	void printMeanStdDev(String fileName, int indicator, double[][][] mean,
		...
		for (int i = 0; i < problemList_.length; i++) {
			...
			if ((Boolean) indicatorMinimize_.get(indicatorList_[indicator])) {// minimize
																				// by
																				// default
	...
	public synchronized int[] getNextProblem() {
		...
		// System.out.println("Prob = " + prob + " Alg = " + alg + " Run = " +
		// runs);
