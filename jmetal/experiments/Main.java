...
public class Main {
	...
	public static void main(String[] args) throws JMException,
		...
		if (args.length == 0) { //
			...
			System.err
					.println("b) jmetal.experiments.Main algorithmName problemName");
			System.err
					.println("c) jmetal.experiments.Main algorithmName problemName paretoFrontFile");
			System.exit(-1);
		} // if
		...
		else if (args.length == 3) { // algorithmName problemName
										// paretoFrontFile
			algorithmName = args[0];
			problemName = args[1];
		...
		if (indicators != null) {
			...
			if (algorithm.getOutputParameter("evaluations") != null) {
				Integer evals = (Integer) algorithm
						.getOutputParameter("evaluations");
