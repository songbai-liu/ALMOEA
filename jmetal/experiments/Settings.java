...
public abstract class Settings {
	 ...
	 */
	public final Algorithm configure(HashMap settings) throws JMException,
			IllegalArgumentException, IllegalAccessException,
			ClassNotFoundException {
		if (settings != null) {
			Field[] fields = this.getClass().getFields();
			for (int i = 0; i < fields.length; i++) {
				if (fields[i].getName().endsWith("_")) { // it is a
															// configuration
															// field
					...
					if (fields[i].getType().equals(int.class)
						...
						if (settings.containsKey(fields[i].getName())) {
							Integer value = (Integer) settings.get(fields[i]
									.getName());
						...
						// double value =
						// Double.parseDouble(experiments.settings.getProperty(fields[i].getName(),""+fields[i].getDouble(this)));
						Double value = (Double) settings.get(fields[i]
								.getName());
						...
						if (settings.containsKey(fields[i].getName())) {
							if (fields[i].getName().equals(
								...
								if ((problem_.getSolutionType().getClass() == RealSolutionType.class)
										|| (problem_.getSolutionType()
												.getClass() == ArrayRealSolutionType.class)) {
									value = 1.0 / problem_
											.getNumberOfVariables();
								} else if (problem_.getSolutionType()
										.getClass() == BinarySolutionType.class
										|| problem_.getSolutionType()
												.getClass() == BinaryRealSolutionType.class) {
									int length = problem_.getNumberOfBits();
									value = 1.0 / length;
								} else {
									int length = 0;
									for (int j = 0; j < problem_
											.getNumberOfVariables(); j++) {
						...
						// Object value =
						// experiments.settings.getProperty(fields[i].getName(),
						// null) ;
						Object value = settings.get(fields[i].getName());
						if (value != null) {
							if (fields[i].getType().equals(Crossover.class)) {
								Object value2 = CrossoverFactory
										.getCrossoverOperator((String) value,
												settings);
								value = value2;
							}

							if (fields[i].getType().equals(Mutation.class)) {
								Object value2 = MutationFactory
										.getMutationOperator((String) value,
												settings);
								value = value2;
							}
			...
			// parameter. Those fields representing crossover and mutations
			// should also
			// be initialized. However, there is still mandatory to configure
			// them
			for (int i = 0; i < fields.length; i++) {
				if (fields[i].getType().equals(Crossover.class)
					...
					String aux = fields[i].getName().substring(0,
							tmp.length() - 1);

					for (int j = 0; j < fields.length; j++) {
						if (i != j) {
							if (fields[j].getName().startsWith(aux)) {
								// The field is a configuration parameter of the
								// crossover
								tmp = fields[j].getName().substring(
										aux.length(),
										fields[j].getName().length() - 1);

								if ((fields[j].get(this) != null)) {
									if (fields[j].getType().equals(int.class)
											|| fields[j].getType().equals(
													Integer.class)) {
										operator.setParameter(tmp,
												fields[j].getInt(this));
									} else if (fields[j].getType().equals(
											double.class)
											|| fields[j].getType().equals(
													Double.class)) {
										operator.setParameter(tmp,
												fields[j].getDouble(this));
									}
								}
							}
						}
			...
			// At this point, we should compare if the pareto front have been
			// added
