...
public class RBoxplot {
	/**
	 * This method produces R scripts for generating eps files containing
	 ...
	 */
	public static void generateScripts(int rows, int cols, String[] problems,
		...
		for (int indicator = 0; indicator < experiment.indicatorList_.length; indicator++) {
			...
			String rFile = rDirectory + "/" + prefix + "."
					+ experiment.indicatorList_[indicator] + ".Boxplot.R";
			...
			for (int i = 0; i < experiment.algorithmNameList_.length; i++) {
				...
				os.write("file" + experiment.algorithmNameList_[i]
						+ "<-paste(file" + experiment.algorithmNameList_[i]
						+ ", " + "indicator, sep=\"/\")" + "\n");
				os.write(experiment.algorithmNameList_[i] + "<-scan(" + "file"
			...
			os.write("\""
					+ experiment.algorithmNameList_[experiment.algorithmNameList_.length - 1]
					+ "\")" + "\n");
