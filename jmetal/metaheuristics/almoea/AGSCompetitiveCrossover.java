...
public class AGSCompetitiveCrossover {
    ...
    public AGSCompetitiveCrossover(SolutionSet population, Problem problem){
    ...
    public AGSCompetitiveCrossover(SolutionSet population, Problem problem, MLPModel currentMLPModel){
        this.population = population;
        this.problem = problem;
        this.numVars = problem.getNumberOfVariables();
        this.numObjs = problem.getNumberOfObjectives();
        ...
        for (int i = 0; i < st[1].size(); i++) {
            ...
            for (int j = 0; j < numVars; j++) {
                if (PseudoRandom.randDouble(0, 1) < CR || j == jrand) {
                    ...
                    value = xParent1.getValue(j) + 0.5*(-xLearning[j] + xParent1.getValue(j)) 
            ...
            for (int j = 0; j < numVars; j++) {
                if (PseudoRandom.randDouble(0, 1) < CR || j == jrand) {
                    ...
                    value = xParent2.getValue(j) + F*(xParent0.getValue(j) - xParent1.getValue(j)) 
                    		+ 0.5*(-xLearning[j] + xParent2.getValue(j));
                    if (value < lowBounds[j])
                        value = lowBounds[j];
                    if (value > upBounds[j])
                        value = upBounds[j];
