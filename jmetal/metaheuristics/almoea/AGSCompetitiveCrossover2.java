...
public class AGSCompetitiveCrossover2 {
    ...
    public AGSCompetitiveCrossover2(SolutionSet population, Problem problem){
    ...
    public AGSCompetitiveCrossover2(SolutionSet population, Problem problem, MLPModel currentMLPModel){
        this.population = population;
        this.problem = problem;
        this.numVars = problem.getNumberOfVariables();
        this.numObjs = problem.getNumberOfObjectives();
    ...
    public SolutionSet reproduction(SolutionSet[] st) throws JMException, ClassNotFoundException {
        ...
        for (int i = 0; i < st[1].size(); i++) {
            offSpring = new Solution(st[1].get(i));
            int rdInt1 = PseudoRandom.randInt(0, st[0].size()-1);
            parents[0] = st[0].get(rdInt1);
            parents[1] = st[1].get(i);

            XReal xChild = new XReal(offSpring);
            XReal xParent0 = new XReal(parents[0]);
            XReal xParent1 = new XReal(parents[1]);
            XReal xCurrent = new XReal(st[1].get(i));
            xLearning = learningViaDAE(xChild);
            int jrand = PseudoRandom.randInt(0, numVars - 1);
            for (int j = 0; j < numVars; j++) {
                if (PseudoRandom.randDouble(0, 1) < CR || j == jrand) {
                    double value;
                    double r1 = 1.0;
                    if(PseudoRandom.randDouble(0, 1) < 1.0) {
                    	r1 = -1.0;
                    }
                    value=PseudoRandom.randDouble(0, 1);
                    value = xParent1.getValue(j) + r1*0.5*(-xLearning[j] + xParent1.getValue(j)) 
        ...
        for (int i = 0; i < st[0].size(); i++) {
            offSpring = new Solution(st[0].get(i));
            int rdInt1 = PseudoRandom.randInt(0, st[0].size()-1);
            while(rdInt1 == i) {
                rdInt1 = PseudoRandom.randInt(0, st[0].size()-1);
            }
            int rdInt2 = PseudoRandom.randInt(0, st[0].size()-1);
            while(rdInt1 == rdInt2 || rdInt2 == i) {
                rdInt2 = PseudoRandom.randInt(0, st[0].size()-1);
            }
            parents[0] = st[0].get(rdInt1);
            parents[1] = st[0].get(rdInt2);
            parents[2] = st[0].get(i);
            XReal xChild = new XReal(offSpring);
            XReal xParent0 = new XReal(parents[0]);
            XReal xParent1 = new XReal(parents[1]);
            XReal xParent2 = new XReal(parents[2]);
            XReal xCurrent = new XReal(st[0].get(i));
            xLearning = learningViaDAE(xChild);
            int jrand = PseudoRandom.randInt(0, numVars - 1);
            ...
            for (int j = 0; j < numVars; j++) {
                if (PseudoRandom.randDouble(0, 1) < CR || j == jrand) {
                    ...
                    value = xParent2.getValue(j) + F*(xParent0.getValue(j) - xParent1.getValue(j)) 
                    		+ r1*0.5*(-xLearning[j] + xParent2.getValue(j));
                    if (value < lowBounds[j])
                        value = lowBounds[j];
                    if (value > upBounds[j])
                        value = upBounds[j];
    ...
    public double[] learningViaDAE(XReal xsol) throws JMException, ClassNotFoundException {
        ...
        for(int var=0;var<numVars;var++) {
            ...
            if(value < lowBounds[var]) {
                value = lowBounds[var] + 0.00001;
            }
            if(value > upBounds[var]) {
                value = upBounds[var] - 0.00001;
            }
            output[var] = value;
        }
        return output;
