...
public class AMOEAD_Hybrid extends Algorithm {
     ...
     *            Problem to solve
     */
    public AMOEAD_Hybrid(Problem problem) {
        super(problem);
    } // MOEAD
    ...
    public SolutionSet execute() throws JMException, ClassNotFoundException {
        ...
        do {
            ...
            nr_[t] = (double)rank.getSubfront(0).size()/populationSize_;
            
            if(evaluations_ < 0.1*maxEvaluations){
            	AGSCompetitiveCrossover2 crossover = new AGSCompetitiveCrossover2(population,problem_,MLP);
            	...
            	if(flag == -1) {
            		if(PseudoRandom.randDouble(0, 1) < 0.75) {
            			GroupingDECrossover crossover = new GroupingDECrossover(population,problem_, groupsize);
            			offspringPopulation = crossover.doCrossover();
            		}else {
            			GroupingSBXCrossover crossover = new GroupingSBXCrossover(population,problem_, groupsize);
            			offspringPopulation = crossover.doCrossover();
            		}
            		...
            		if(PseudoRandom.randDouble(0, 1) < 0.75) {
            			...
            			SBXCrossover crossover = new SBXCrossover(population,problem_);
                        offspringPopulation = crossover.doCrossover();
            		}
            	}
            }
        ...
        printMetric("UR_"+ problem_.getNumberOfObjectives()+"Obj_"+problem_.getName()+
                "_" + problem_.getNumberOfVariables() + "D" +".txt", ur_);
        printMetric("NR_"+ problem_.getNumberOfObjectives()+"Obj_"+problem_.getName()+
                "_" + problem_.getNumberOfVariables() + "D" +".txt", nr_);
