...
public class GroupingDECrossover {
    ...
    public GroupingDECrossover(SolutionSet population, Problem problem){
    ...
    public GroupingDECrossover(SolutionSet population, Problem problem, int groupSize){
        this.population = population;
        this.problem = problem;
        this.numVars = problem.getNumberOfVariables();
        this.numObjs = problem.getNumberOfObjectives();
	...
	public double[] searchInTransferedSpace(double[][] encodedParents, double learningRate) {
		...
		for(int j=0;j<groupSize_;j++) {
			newEncode[j] = encodedParents[0][j] + 0.5*(encodedParents[1][j] - encodedParents[2][j]);
