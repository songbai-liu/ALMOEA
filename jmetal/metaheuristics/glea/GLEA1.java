...
public class GLEA1 extends Algorithm{
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(generations_ <= maxGenerations_){
			if((generations_) % (interval_) == 0) { randomGrouping(); } 
	...
	public void groupBroadLearning(SolutionSet s1, SolutionSet s2) throws JMException {
		...
		for (int i = 0; i < s2.size(); i++) {
			...
			if(PseudoRandom.randDouble() < 0.1) {//broad learning
				...
				for(int j=0;j<groupSize_;j++){
					double val;
					for(int m=0;m<group_[j].length;m++){
						val = xChild.getValue(group_[j][m]) + 0.5*(xParent.getValue(group_[j][m]) - xChild.getValue(group_[j][m]));
						if (val < xChild.getLowerBound(group_[j][m]))
							val = xChild.getLowerBound(group_[j][m]);
						if (val > xChild.getUpperBound(group_[j][m]))
							val = xChild.getUpperBound(group_[j][m]);
	...
	public void groupWeightLearning(SolutionSet s1) throws JMException {
		...
		for (int i = 0; i < s1.size(); i++) {
			...
			if(PseudoRandom.randDouble() < 0.9) {//search in the weighted space
				r1 = PseudoRandom.randDouble(0.3, 0.6);
				for(int j=0;j<groupSize_;j++){
					weight[0][j] = 0;
					weight[1][j] = 0;
					weight[2][j] = 0;
				...
				double val;
				for(int d=0;d<problem_.getNumberOfVariables();d++) {
					val = xChild.getValue(d) + PseudoRandom.randDouble(0.3, 0.6)*(ss1.getValue(d) - ss2.getValue(d));
					if (val < xChild.getLowerBound(d))
						val = xChild.getLowerBound(d);
					if (val > xChild.getUpperBound(d))
						val = xChild.getUpperBound(d);
					xChild.setValue(d, val);
				}
			}//if
			mutationOperator_.execute(child);
			problem_.evaluate(child);
