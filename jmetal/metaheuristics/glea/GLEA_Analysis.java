...
public class GLEA_Analysis extends Algorithm{
	...
	double[] UB_;
	double[] LB_;
	public GLEA_Analysis(Problem problem) {
		super(problem);
		numObj_ = problem.getNumberOfObjectives();
		numVar_ = problem.getNumberOfVariables();
	}
	
	double[] IGD = new double[25];
	QualityIndicator indicators = new QualityIndicator(problem_,"D:\\Matlab\\TruePF\\LMF\\" + problem_.getHtype() + "_" + problem_.getNumberOfObjectives() +"D" +".pf");
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while(generations_ <= maxGenerations_){
			...
			if(generations_ % 136 == 0) {
				NondominatedRanking ranking = new NondominatedRanking(population_);
				IGD[gen]=indicators.getIGD1(ranking.getSubfront(0));
				gen++;
			}
			reproduction();
		...
		printGD(problem_.getName()+ "_IGD" +".txt",IGD);
