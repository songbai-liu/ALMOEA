 ...
 */
public class SMPSO1 extends Algorithm {
	...
	public SMPSO1(Problem problem, Vector<Double> variables,
	 ...
	 *            Problem to solve
	 */
	public SMPSO1(Problem problem, String trueParetoFront)
	...
	public void initParams() {
		...
		leaders_ = new CrowdingArchive(archiveSize_,
				problem_.getNumberOfObjectives());
	...
	private void computeSpeed(int iter, int miter) throws JMException,
		...
		for (int i = 0; i < swarmSize_; i++) {
			...
			// Select a global best_ for calculate the speed of particle i,
			// bestGlobal
			Solution one, two;
			...
			for(int len=0;len<20;len++) {
				Solution s1, s2;
				int rr1 = PseudoRandom.randInt(0, leaders_.size() - 1);
				int rr2 = PseudoRandom.randInt(0, leaders_.size() - 1);
				s1 = leaders_.get(rr1);
				s2 = leaders_.get(rr2);

				if (crowdingDistanceComparator_.compare(s1, s2) < 1) {
					bestGlobal = new XReal(s1);
				} else {
					bestGlobal = new XReal(s2);
					// Params for velocity equation
				}
				for(int var=len*10;var<(len*10+10);var++) {
					speed_[i][var] = velocityConstriction(
							constrictionCoefficient(C1, C2)
									* (inertiaWeight(iter, miter, wmax, wmin)  //may be W
											* speed_[i][var]
											+ C1
											* r1
	...
	private void mopsoMutation(int actualIteration, int totalIterations)
		...
		for (int i = 0; i < particles_.size(); i++) {
			...
			// if (i % 3 == 0) { //particles_ mutated with a non-uniform
			// mutation %3
			// nonUniformMutation_.execute(particles_.get(i));
			// } else if (i % 3 == 1) { //particles_ mutated with a uniform
			// mutation operator
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (iteration_ < maxIterations_) {
			...
			for (int i = 0; i < particles_.size(); i++) {
				...
				if (flag != 1) { // the new particle is best_ than the older
									// remeber
					Solution particle = new Solution(particles_.get(i));
					best_[i] = particle;
				}
