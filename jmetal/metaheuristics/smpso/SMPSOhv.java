...
public class SMPSOhv extends Algorithm {
	 ...
	 * Stores the leaders_
	 */
	private HypervolumeArchive leaders_;
	/**
	...
	public SMPSOhv(Problem problem, Vector<Double> variables,
	...
	public void initParams() {
		...
		leaders_ = new HypervolumeArchive(archiveSize_,
				problem_.getNumberOfObjectives());
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		// problem_.getNumberOfObjectives());
		leaders_.actualiseHVContribution();

		// -> Step 7. Iterations ..
		while (iteration_ < maxIterations_) {
			System.out.println("Iteration: " + iteration_);
			try {
				// Compute the speed_
				computeSpeed(iteration_, maxIterations_);
			} catch (IOException ex) {
				Logger.getLogger(SMPSOhv.class.getName()).log(Level.SEVERE,
						null, ex);
			}
