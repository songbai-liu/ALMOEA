 ...
 */
public class AdaptiveDECrossover extends Crossover {
	/**
	 ...
	 * Constructor
	 */
	public AdaptiveDECrossover(HashMap<String, Object> parameters) {
		super(parameters);
	...
	public Object execute(Object object) throws JMException {
		...
		int numberGroups = segment.length;
        
		for(int g=0; g<numberGroups; g++){
			double rd = PseudoRandom.randDouble(0.0, 1.0);	
			double rv = (0.8*g)/numberGroups;
			if(rd >= rv){
				int numberOfVariables = segment[g].length;
				...
				if ((DE_Variant_.compareTo("rand/1/bin") == 0) || (DE_Variant_.compareTo("best/1/bin") == 0)) {
					for (int var = 0; var < numberOfVariables; var++) {
						...
						if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
							...
							value = xCurrent.getValue(j) + 0.5*(xParent0.getValue(j) - xParent1.getValue(j));
