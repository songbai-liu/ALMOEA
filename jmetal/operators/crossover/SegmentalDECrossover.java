 ...
 */
public class SegmentalDECrossover extends Crossover {
	/**
	 ...
	 */
	private static final double DEFAULT_K = 0.05;
	 ...
	 * Constructor
	 */
	public SegmentalDECrossover(HashMap<String, Object> parameters) {
		super(parameters);
	...
	public Object execute(Object object) throws JMException {
		...
		double rd1 = PseudoRandom.randDouble(0.3, 0.7);
		double rd2 = PseudoRandom.randDouble(0.1, 0.9);

		// STEP 4. Checking the DE variant
		if ((DE_Variant_.compareTo("rand/1/bin") == 0) || (DE_Variant_.compareTo("best/1/bin") == 0)) {
			for (int var = 0; var < numberOfVariables; var++) {
				int j = segment[var];
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == var) {
					double value;
					value=PseudoRandom.randDouble(0, 1);
					value = xParent0.getValue(j) + 0.5*(xParent1.getValue(j) - xParent2.getValue(j));
							//+0.5*(xParent2.getValue(j) - xParent0.getValue(j));
