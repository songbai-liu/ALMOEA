 ...
 * solutions.
 */
public class SegmentalSBXCrossover extends Crossover {
	/**
	 ...
	 */
	public SegmentalSBXCrossover(HashMap<String, Object> parameters) {
		super(parameters);
	...
	public Solution[] doCrossover(double probability, Solution parent1,
			Solution parent2, int[] segment) throws JMException {
		...
		int numberOfVariables = segment.length;

		if (PseudoRandom.randDouble() <= probability) {
			for (i = 0; i < numberOfVariables; i++) {
				valueX1 = x1.getValue(segment[i]);
				valueX2 = x2.getValue(segment[i]);
				if (PseudoRandom.randDouble() <= 0.5) {
					if (java.lang.Math.abs(valueX1 - valueX2) > EPS) {
						...
						yL = x1.getLowerBound(segment[i]);
						yu = x1.getUpperBound(segment[i]);
						...
						if (PseudoRandom.randDouble() <= 0.5) {
							offs1.setValue(segment[i], c2);
							offs2.setValue(segment[i], c1);
						} else {
							offs1.setValue(segment[i], c1);
							offs2.setValue(segment[i], c2);
						} // if
					} else {
						offs1.setValue(segment[i], valueX1);
						offs2.setValue(segment[i], valueX2);
					} // if
				} else {
					offs1.setValue(segment[i], valueX2);
					offs2.setValue(segment[i], valueX1);
				} // if
			} // if
		} // if
	...
	public Object execute(Object object) throws JMException {
		...
		int[] segment = (int[])parameters[1];

		if (parents.length != 2) {
			Configuration.logger_
					.severe("SBXCrossover.execute: operator needs two " + "parents");
		...
		Solution[] offSpring;
		offSpring = doCrossover(crossoverProbability_, parents[0], parents[1],segment);
