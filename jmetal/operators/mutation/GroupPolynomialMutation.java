 ...
 */
public class GroupPolynomialMutation extends Mutation {
	 ...
	 */
	public GroupPolynomialMutation(HashMap<String, Object> parameters) {
		super(parameters);
	...
	public void doMutation(double probability, Solution solution, int[][] group)
			throws JMException {
		double rnd, delta1, delta2, mut_pow, deltaq;
		double y, yl, yu, val, xy;
		XReal x = new XReal(solution);
		for(int g=0;g<group.length;g++){
			if (PseudoRandom.randDouble() <= probability) {
				for(int var=0;var<group[g].length;var++){
					y = x.getValue(group[g][var]);
					yl = x.getLowerBound(group[g][var]);
					yu = x.getUpperBound(group[g][var]);
					delta1 = (y - yl) / (yu - yl);
					delta2 = (yu - y) / (yu - yl);
					rnd = PseudoRandom.randDouble();
					mut_pow = 1.0 / (eta_m_ + 1.0);
	...
	public Object execute(Object object) throws JMException {
		...
		Solution solution = (Solution) parameters[0];
		int[][] group = (int[][]) parameters[1];

		if (!VALID_TYPES.contains(solution.getType().getClass())) {
			Configuration.logger_
					.severe("GroupPolynomialMutation.execute: the solution "
		...
		mutationProbability_ = 1.0/group[0].length;
		doMutation(mutationProbability_, solution, group);
		return solution;
	} // execute

} // GroupPolynomialMutation
