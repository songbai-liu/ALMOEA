 ...
 */
public class SegmentalMutation extends Mutation {
	 ...
	 */
	public SegmentalMutation(HashMap<String, Object> parameters) {
		super(parameters);
	 ...
	 * @throws JMException
	 */
	public void doMutation(double probability, Solution solution, int[] segment)
			throws JMException {
		...
		for (int var = 0; var < numberOfVariables; var++) {
			if (PseudoRandom.randDouble() <= probability) {
				y = x.getValue(segment[var]);
				yl = x.getLowerBound(segment[var]);
				yu = x.getUpperBound(segment[var]);
				delta1 = (y - yl) / (yu - yl);
				delta2 = (yu - y) / (yu - yl);
	...
	public Object execute(Object object) throws JMException {
		...
		mutationProbability_ = 1.0/segment.length;
		doMutation(mutationProbability_, solution, segment);
		return solution;
	} // execute
