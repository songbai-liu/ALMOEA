...
public class LSMOP7 extends LSMOP {
	 ...
	 */
	public LSMOP7(String solutionType) throws ClassNotFoundException {
	 ...
	 *            The solution type must "Real" or "BinaryReal".
	 */
	public LSMOP7(String solutionType, Integer numberOfVariables,
	...
	public void evaluate(Solution solution) throws JMException {
		...
		if(dim != numberOfVariables_) {
			System.out.println("LSMOP7: Recent Dim = "+ dim);
			System.out.println("LSMOP7: The Preset numberOfVariables = "+ numberOfVariables_);
		}
		...
		for (int i = 0; i < numberOfObjectives_; i++) {
			...
			if(i%2 == 0){
				for(int j=0;j<nk;j++){
					...
					gx[i] += (cg+cs+20+Math.exp(1))/sublen[i];
				}
			}else{
				for(int j=0;j<nk;j++){
					int t = len[i]+numberOfObjectives_-1+j*sublen[i];
