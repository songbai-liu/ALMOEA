...
public class Transformations {
	...
	public float b_flat(float y, float A, float B, float C) {
		float tmp1 = Math.min((float) 0, (float) Math.floor(y - B)) * A
				* (B - y) / B;
		float tmp2 = Math.min((float) 0, (float) Math.floor(C - y)) * (1 - A)
				* (y - C) / (1 - C);
	...
	public float s_linear(float y, float A) {
		return correct_to_01(Math.abs(y - A)
				/ (float) Math.abs(Math.floor(A - y) + A));
	} // s_linear
	...
	public float s_decept(float y, float A, float B, float C) {
		...
		tmp1 = (float) Math.floor(y - A + B) * ((float) 1.0 - C + (A - B) / B)
				/ (A - B);
		tmp2 = (float) Math.floor(A + B - y)
				* ((float) 1.0 - C + ((float) 1.0 - A - B) / B)
				/ ((float) 1.0 - A - B);
	...
	public float r_nonsep(float[] y, int A) {
		...
		denominator = y.length * tmp
				* ((float) 1.0 + (float) 2.0 * A - (float) 2.0 * tmp) / A;
		numerator = (float) 0.0;
