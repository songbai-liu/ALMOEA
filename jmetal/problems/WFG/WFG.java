...
/**
 * Implements a reference abstract class for all WFG test problems Reference:
 * Simon Huband, Luigi Barone, Lyndon While, Phil Hingston A Scalable
...
public abstract class WFG extends Problem {
	 ...
	 */
	public WFG(String solutionType, Integer k, Integer l, Integer M) {
		this.k_ = k;
		this.l_ = l;
		this.M_ = M;
	...
	public float[] calculate_x(float[] t) {
		...
		for (int i = 0; i < M_ - 1; i++) {
			x[i] = Math.max(t[M_ - 1], A_[i]) * (t[i] - (float) 0.5)
					+ (float) 0.5;
		}
