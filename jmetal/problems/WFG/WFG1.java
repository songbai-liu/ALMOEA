...
/**
 * This class implements the WFG1 problem Reference: Simon Huband, Luigi Barone,
...
public class WFG1 extends WFG {
	...
	/**
	 * Constructor Creates a default WFG1 instance with 2 position-related
	 * parameters 4 distance-related parameters and 2 objectives
	 * 
	 * @param solutionType
	...
	public float[] evaluate(float[] z) {
		...
		for (int m = 1; m <= M_ - 1; m++) {
			result[m - 1] = D_ * x[M_ - 1] + S_[m - 1]
					* (new Shapes()).convex(x, m);
		}

		result[M_ - 1] = D_ * x[M_ - 1] + S_[M_ - 1]
				* (new Shapes()).mixed(x, 5, (float) 1.0);
	...
	public float[] t2(float[] z, int k) {
		...
		for (int i = k; i < z.length; i++) {
			result[i] = (new Transformations()).b_flat(z[i], (float) 0.8,
					(float) 0.75, (float) 0.85);
		}
