...
/**
 * Creates a default WFG9 problem with 2 position-related parameters, 4
 * distance-related parameters, and 2 objectives
 */
public class WFG9 extends WFG {

	/**
	 * Creates a default WFG9 with 2 position-related parameters, 4
	 * distance-related parameters, and 2 objectives
	 * 
	 * @param solutionType
	 ...
	 * @param l
	 *            Number of distance variables
	 * @param M
	 *            Number of objective functions
	 * @param solutionType
	 *            The solution type must "Real" or "BinaryReal".
	 */
	public WFG9(String solutionType, Integer k, Integer l, Integer M)
	...
	public float[] evaluate(float[] z) {
		...
		for (int m = 1; m <= M_; m++) {
			result[m - 1] = D_ * x[M_ - 1] + S_[m - 1]
					* (new Shapes()).concave(x, m);
		}
		return result;
	} // evaluate
	...
	public float[] t1(float[] z, int k) {
		...
		for (int i = 0; i < z.length - 1; i++) {
			...
			result[i] = (new Transformations()).b_param(z[i], aux, (float) 0.98
					/ (float) 49.98, (float) 0.02, (float) 50);
		}
	...
	public float[] t2(float[] z, int k) {
		...
		for (int i = 0; i < k; i++) {
			result[i] = (new Transformations()).s_decept(z[i], (float) 0.35,
					(float) 0.001, (float) 0.05);
		}

		for (int i = k; i < z.length; i++) {
			result[i] = (new Transformations()).s_multi(z[i], 30, 95,
					(float) 0.35);
		}
