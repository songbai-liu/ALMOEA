 ...
 * <trueFrontFile> <numberOfObjectives> Reference: A. Zhou, Y. Jin, Q. Zhang, B.
 * Sendhoff, and E. Tsang Combining model-based and genetics-based offspring
...
public class GeneralizedSpread {
	...
	public static jmetal.qualityIndicator.util.MetricsUtil utils_; // MetricsUtil
																	// provides
																	// some
	...
	public double generalizedSpread(double[][] paretoFront,
		...
		maximumValue = utils_.getMaximumValues(paretoTrueFront,
				numberOfObjectives);
		minimumValue = utils_.getMinimumValues(paretoTrueFront,
				numberOfObjectives);

		normalizedFront = utils_.getNormalizedFront(paretoFront, maximumValue,
				minimumValue);
		...
		for (int i = 0; i < numberOfObjectives; i++) {
			Arrays.sort(normalizedParetoFront,
					new jmetal.qualityIndicator.util.ValueComparator(i));
			System.arraycopy(
					normalizedParetoFront[normalizedParetoFront.length - 1], 0,
					extremValues[i], 0, numberOfObjectives);
		}
		...
		if (utils_.distance(normalizedFront[0],
				normalizedFront[normalizedFront.length - 1]) == 0.0) {
			return 1.0;
		} else {
			...
			for (double[] aNormalizedFront : normalizedFront) {
				dmean += utils_.distanceToNearestPoint(aNormalizedFront,
						normalizedFront);
			}
			...
			for (double[] extremValue : extremValues) {
				dExtrems += utils_.distanceToClosedPoint(extremValue,
						normalizedFront);
			}
			...
			double mean = 0.0;
			for (double[] aNormalizedFront : normalizedFront) {
				mean += Math.abs(utils_.distanceToNearestPoint(
						aNormalizedFront, normalizedFront) - dmean);
			}

			double value = (dExtrems + mean)
