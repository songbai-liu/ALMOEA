...
public class Hypervolume1 {
  ...
  */
   ...
   */
  public double hypervolume1(double [][] paretoFront, 
    ...
    for(int i = 0; i < numberOfObjectives; i++){
    	if(fun == 6){
			...
			if(i!=numberOfObjectives-1){
				maximumValues[numberOfObjectives-1-i] = Math.pow(Math.sqrt(2)/2, i);
			}else{
				maximumValues[0] = maximumValues[1];
			}
		}else if(fun>12&fun<=21){
			maximumValues[i] = 2*(i+1);
		}
    	minimumValues[i] = 0.0;
    }
  ...
  public static void main(String args[]) {
    ...
    Hypervolume1 qualityIndicator = new Hypervolume1();
    //Read the front from the files
    double [][] solutionFront = qualityIndicator.utils_.readFront(args[0]);

    
    //Obtain delta value
  //  double value = qualityIndicator.hypervolume1(solutionFront,
                                            //new Integer(args[2]));
