...
public class R2 {
	 ...
	 */
	private double R2Withouth(double[][] approximation, double[][] paretoFront,
			int index) {
		...
		for (int i = 0; i < approximation.length; i++) {
			for (int j = 0; j < lambda_.length; j++) {
				matrix_[i][j] = lambda_[j][0]
						* Math.abs(normalizedApproximation[i][0]);
				for (int n = 1; n < nObj_; n++) {
					matrix_[i][j] = Math.max(matrix_[i][j], lambda_[j][n]
							* Math.abs(normalizedApproximation[i][n]));
				}
			}
		}
		// }
	 ...
	 *            A vector containing the lambda vectors for R2
	 */
	public int[] getNBest(double[][] approximation, double[][] paretoFront,
			int N) {
	 ...
	 * 
	 * @param SolutionSet
	 *            the solution set for which the best solutions are computed
	 */
	 ...
	 * point 0, 0. Normalization is using taking into account the population
	 * itself
	 * 
	 * @param set
	 * @return
	...
	public static void main(String args[]) {
		...
		double[][] approximationFront = qualityIndicator.utils_
				.readFront(args[0]);
		...
		System.out.println(qualityIndicator.R2Withouth(approximationFront,
				paretoFront, 1));
		System.out.println(qualityIndicator.R2Withouth(approximationFront,
				paretoFront, 15));
		System.out.println(qualityIndicator.R2Withouth(approximationFront,
