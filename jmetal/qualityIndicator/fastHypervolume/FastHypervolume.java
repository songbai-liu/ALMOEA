...
public class FastHypervolume {
	...
	public double computeHypervolume(SolutionSet solutionSet) {
		...
		if (solutionSet.size() == 0)
			...
			if (numberOfObjectives_ == 2) {
				solutionSet.sort(new ObjectiveComparator(
						numberOfObjectives_ - 1, true));
				hv = get2DHV(solutionSet);
			} else {
				updateReferencePoint(solutionSet);
				Front1 front = new Front1(solutionSet.size(),
						numberOfObjectives_, solutionSet);
				hv = new WFGHV1(numberOfObjectives_, solutionSet.size(),
						referencePoint_).getHV(front);
			}
		}
	...
	public double computeHypervolume(SolutionSet solutionSet,
		...
		if (solutionSet.size() == 0)
			...
			numberOfObjectives_ = solutionSet.get(0).numberOfObjectives();
			...
			if (numberOfObjectives_ == 2) {
				...
				WFGHV1 wfg = new WFGHV1(numberOfObjectives_, solutionSet.size());
				Front1 front = new Front1(solutionSet.size(),
						numberOfObjectives_, solutionSet);
	...
	public double get2DHV(SolutionSet solutionSet) {
		double hv = 0.0;
		if (solutionSet.size() > 0) {
			hv = Math.abs((solutionSet.get(0).getObjective(0) - referencePoint_
					.getObjective(0))
					* (solutionSet.get(0).getObjective(1) - referencePoint_
							.getObjective(1)));

			for (int i = 1; i < solutionSet.size(); i++) {
				double tmp = Math
						.abs((solutionSet.get(i).getObjective(0) - referencePoint_
								.getObjective(0))
								* (solutionSet.get(i).getObjective(1) - solutionSet
										.get(i - 1).getObjective(1)));
				hv += tmp;
			}
		}
		return hv;
	...
	public void computeHVContributions(SolutionSet solutionSet) {
		...
		for (int i = 0; i < solutionSet.size(); i++) {
			...
			if (numberOfObjectives_ == 2) {
				...
				Front1 front = new Front1(solutionSet.size(),
						numberOfObjectives_, solutionSet);
				double hv = new WFGHV1(numberOfObjectives_, solutionSet.size(),
						referencePoint_).getHV(front);
