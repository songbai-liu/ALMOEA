...
/**
 * Created with IntelliJ IDEA. User: Antonio J. Nebro Date: 25/07/13 Time: 17:50
...
public class WFGHV1 {
	...
	public WFGHV1(int dimension, int maxNumberOfPoints) {
		...
		for (int i = 0; i < maxd; i++) {
			fs_[i] = new Front1(maxNumberOfPoints, dimension);
		}
	}

	public WFGHV1(int dimension, int maxNumberOfPoints, Solution referencePoint) {
	...
	public WFGHV1(int dimension, int maxNumberOfPoints, Point referencePoint) {
	...
	public double getHV(Front1 front, Solution referencePoint) {
		referencePoint_ = new Point(referencePoint);
		double volume = 0.0;
		sort(front);
		...
		if (currentDimension_ == 2)
			...
			for (int i = front.nPoints_ - 1; i >= 0; i--) {
				volume += Math
						.abs(front.getPoint(i).objectives_[currentDimension_]
	...
	public double getHV(Front1 front) {
		double volume = 0.0;
		sort(front);
	...
	public double get2DHV(Front1 front) {
		double hv = 0.0;

		hv = Math
				.abs((front.getPoint(0).getObjectives()[0] - referencePoint_.objectives_[0])
	...
	public double getInclusiveHV(Point p) {
		...
		for (int i = 0; i < currentDimension_; i++) {
			volume *= Math.abs(p.objectives_[i]
					- referencePoint_.objectives_[i]);
		}

		return volume;
	}

	public double getExclusiveHV(Front1 front, int point) {
		double volume;
	...
	public static void main(String args[]) throws IOException {
		...
		WFGHV1 wfghv = new WFGHV1(referencePoint.getNumberOfObjectives(),
				front.getNumberOfPoints(), referencePoint);
