...
public class DifferentialEvolutionOffspringPolynomial extends Offspring {
	...
	private DifferentialEvolutionOffspringPolynomial(
			double mutationProbability, double distributionIndexForMutation)
			throws JMException {
		HashMap parameters;
	...
	public Solution getOffspring(SolutionSet solutionSet) {
		...
		try {
			...
			Logger.getLogger(
					DifferentialEvolutionOffspringPolynomial.class.getName())
	...
	public Solution getOffspring(SolutionSet solutionSet, SolutionSet archive) {
		...
		try {
			...
			offSpring = new Solution(new Solution(
					(Solution) selection_.execute(solutionSet)));
