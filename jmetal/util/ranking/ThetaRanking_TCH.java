...
public class ThetaRanking_TCH implements Ranking {
	...
	public ThetaRanking_TCH(SolutionSet solutionSet,  double[][] lambda, 
			double[] zideal, 
	...
	void associate() {
		...
		for (int k = 0; k < solutionSet_.size(); k++) {
			...
			double d = getTCHDistance(sol);
			//setFitness(sol, index, d1, d2);
			setFitness(sol, index, d, 0.0);
	...
	double getDistance(Solution sol) {
		double ip = 0;
		double refLenSQ = 0;
		...
		for (int j = 0; j < obj_; j++) {
			...
			ip += sol.getNormalizedObjective(j) * 1.0;
			refLenSQ += (1.0 * 1.0);
		}
		refLenSQ = Math.sqrt(refLenSQ);

		d = Math.abs(ip) / refLenSQ;
		return d;
	}
	
	double getTCHDistance(Solution sol) {

		double d = sol.getNormalizedObjective(0);
		for (int j = 1; j < obj_; j++) {
			if(d < sol.getNormalizedObjective(j)){
				d = sol.getNormalizedObjective(j);
			}
		}
		return d;
	}
