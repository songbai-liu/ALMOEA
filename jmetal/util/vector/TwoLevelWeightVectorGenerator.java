...
//		System.out.println(N2);
	...
	public static void main(String args[]){
		...
		VectorGenerator vg = new TwoLevelWeightVectorGenerator(2, 0,
				10);
		double[][] lambda = vg.getVectors();
		System.out.println(lambda.length);
		//double sb = Math.pow(10, -6);
		for (int i = 0; i < lambda.length; i++){
			for (int j = 0; j < lambda[i].length; j++){
				if(lambda[i][j] == 0){
					System.out.print(String.format("%E", Math.pow(10, -5)) + "\t");
				}else{
					System.out.print(lambda[i][j] + "\t");
				}
			}
